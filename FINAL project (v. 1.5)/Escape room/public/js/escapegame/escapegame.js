...
function preload() {
  ...
  openDoor.setVolume(1);
  openChest.setVolume(1);
  pickKey.setVolume(1);
}
//preload ends
...
function startScreen(){
  ...
  mouseOverHTP = isMouseOver(htpX,htpY,htpW,htpH);
  fill(textBG);
  if (mouseOverStart) {
    fill(hovered);
  }
...
function mouseClicked() {
  ...
  if (!started && !howToPlayOpen && mouseOverHTP) {
    howToPlayOpen = true;
    return;
  }
  if (howToPlayOpen) {
...
function showHowToPlay() {
  ...
  rect(0,0,canvasX,canvasY);
  fill(textColor);
  const howToPlayText = "Placeholder text";
  text(howToPlayText, 550, canvasY/2);
...
function hoverText() {
  ...
  if (started && currentLocation === locations.hallway && mouseX > cDoorX && mouseX < cDoorX+cDoorW && mouseY > cDoorY && mouseY < cDoorY+cDoorH) {
    text('Circuit Room', mouseX, mouseY-10);
  }
  if (started && currentLocation === locations.hallway && mouseX > eDoorX && mouseX < eDoorX+cDoorW && mouseY > eDoorY && mouseY < eDoorY+eDoorH) {
    text('Exit', mouseX, mouseY-10);
  }
  if (started && currentLocation === locations.hallway && mouseX > sDoorX && mouseX < sDoorX+sDoorW && mouseY > sDoorY && mouseY < sDoorY+sDoorH) {
    text('Storage Room', mouseX, mouseY-10);
  }
  if (started && currentLocation === locations.hallway && mouseX > rDoorX && mouseX < rDoorX+rDoorW && mouseY > rDoorY && mouseY < rDoorY+rDoorH) {
