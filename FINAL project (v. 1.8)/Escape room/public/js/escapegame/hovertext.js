...
function hallwayHover() {
  if (!exitKeyOwned && !showingKeypad && currentLocation === locations.hallway && isMouseOver(exitDoorX,exitDoorY,exitDoorW,exitDoorH) && mouseIsPressed) {
    text("The exit door\nIt won't open", mouseX, mouseY-40);
  }
  if (!showingKeypad && currentLocation === locations.hallway && isMouseOver(hwRadiatorX,hwRadiatorY,hwRadiatorW,hwRadiatorH) && mouseIsPressed) {
    text("An old, broken radiator\nIt's cold to the touch", mouseX, mouseY-40);
  }
  if (!showingKeypad && currentLocation === locations.hallway && isMouseOver(hwWindowX,hwWindowY,hwWindowW,hwWindowH) && mouseIsPressed) {
    text("That window is too high\nfor me to reach", mouseX, mouseY+40);
  }
  if (!showingKeypad && currentLocation === locations.hallway && isMouseOver(hwBloodX,hwBloodY,hwBloodW,hwBloodH) && mouseIsPressed) {
    text("Looks like a blood stain\nSeems it's been there a while", mouseX, mouseY-40);
  }
  if (!showingKeypad && currentLocation === locations.hallway && isMouseOver(hwLight1X,hwLight1Y,hwLight1W,hwLight1H) && mouseIsPressed) {
    text("At least the lights are on", mouseX, mouseY-10);
  }
  if (!showingKeypad && currentLocation === locations.hallway && isMouseOver(hwLight2X,hwLight2Y,hwLight2W,hwLight2H) && mouseIsPressed) {
...
function storageHover() {
  if (!chestKeyOwned && currentLocation === locations.storageRoom && isMouseOver(chestX,chestY,chestW,chestH) && mouseIsPressed) {
    text("The chest has a large\npadlock on it", mouseX, mouseY-40);
  }
  if (currentLocation === locations.storageRoom && isMouseOver(sPL1X,sPL1Y,sPL1W,sPL1H) && mouseIsPressed) {
    text("These padlocks are all\ncompletely busted", mouseX, mouseY-40);
  }
  if (currentLocation === locations.storageRoom && isMouseOver(sPL2X,sPL2Y,sPL2W,sPL2H) && mouseIsPressed) {
    text("These padlocks are all\ncompletely busted", mouseX, mouseY-40);
  }
  if (currentLocation === locations.storageRoom && isMouseOver(sPL3X,sPL3Y,sPL3W,sPL3H) && mouseIsPressed) {
    text("These padlocks are all\ncompletely busted", mouseX, mouseY-40);
  }
  if (currentLocation === locations.storageRoom && isMouseOver(sJunkX,sJunkY,sJunkW,sJunkH) && mouseIsPressed) {
